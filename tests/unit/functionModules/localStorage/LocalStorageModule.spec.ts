...
describe('LocalStorageModule.ts', () => {
  ...
  it('stringifies storeables for save', async () => {
    const details = new DatatrustTaskDetails('0x123', FfaDatatrustTaskType.buyListing)
  ...
  it('returns storeable for key', () => {
    const details = new DatatrustTaskDetails('0x234', FfaDatatrustTaskType.createListing)
    const task = new DatatrustTask('key', details);
