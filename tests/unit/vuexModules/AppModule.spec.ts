...
describe('AppModule.ts', () => {
  ...
  it ('resets', () => {
    appModule.reset()
    expect(appModule.areParametersSet).toBeFalsy()
    expect(appModule.makerPayment).toBe(-1)
    expect(appModule.costPerByte).toBe(-1)
    expect(appModule.stake).toBe(-1)
    expect(appModule.priceFloor).toBe(-1)
    expect(appModule.plurality).toBe(-1)
    expect(appModule.voteBy).toBe(-1)
    expect(appModule.etherTokenBalance).toBe(-1)
    expect(appModule.marketTokenBalance).toBe(-1)
    expect(appModule.etherTokenDatatrustContractAllowance).toBe(-1)
    expect(appModule.etherTokenReserveContractAllowance).toBe(-1)
    expect(appModule.marketTokenVotingContractAllowance).toBe(-1)
